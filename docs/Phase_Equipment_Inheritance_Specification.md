...
### 期待効果
- **作業効率向上**: 機材登録作業時間を70%削減
- **品質向上**: 手動入力ミスの大幅削減
- **運用負荷軽減**: 複数フェーズ管理の負担軽減
...
#### 公演ステータス制約
**継承先公演（継承を受ける側）**:
- ✅ **準備中（preparing）**: 機材リスト作成段階（主要対象）
- ✅ **進行中（ongoing）**: 追加フェーズへの継承
- ❌ **企画中（planning）**: データが不完全
- ❌ **完了（completed）**: 既に終了済み
- ❌ **キャンセル（cancelled）**: 継承不要

**継承元公演（参照される側）**:
- ✅ **完了（completed）**: 実績として最も信頼性が高い（推奨）
- ✅ **進行中（ongoing）**: 現在進行中公演からの継承
- ✅ **準備中（preparing）**: 準備中公演からの継承
- ⚠️ **企画中（planning）**: データ不完全の可能性
- ❌ **キャンセル（cancelled）**: 継承する意味がない
...
### Phase 1: データベース・バックエンド（3-4時間）

#### 1-1. マイグレーション作成（30分）
- `phase_equipment` テーブルの `source_phase_equipment_id` カラム追加
- インデックス追加

#### 1-2. モデル拡張（1時間）
- `PhaseEquipment` モデルの継承関連メソッド追加
- リレーション設定

#### 1-3. コントローラー実装（2-2.5時間）
- `getInheritablePhases()` メソッド実装
- `getEquipmentForInheritance()` メソッド実装
- `inheritEquipment()` メソッド実装
- バリデーション・エラーハンドリング

### Phase 2: フロントエンド（4-5時間）
...
#### 2-2. JavaScript実装（2時間）
- `PhaseEquipmentInheritance` クラス実装
- API通信・エラーハンドリング
- ユーザーインタラクション処理

### Phase 3: テスト・品質保証（2時間）
