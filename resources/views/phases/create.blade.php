...
@section('breadcrumb')
    ...
    > <span class="text-gray-800 dark:text-gray-200">フェーズ作成</span>
@endsection

@section('header')
    <div class="w-full">
        <p class="text-base sm:text-lg text-gray-600 dark:text-gray-400">{{ $performance->title }}</p>
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">フェーズ作成</h1>
...
<div class="p-3 sm:p-6">
    <form id="create-form" method="POST" action="{{ route('performances.phases.store', $performance) }}" class="space-y-4 sm:space-y-6"
          x-data="{ startDate: '{{ old('start_date') }}' }">
        @csrf
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4 space-y-4 sm:space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" id="end_date" value="{{ old('end_date') }}" :min="startDate" required
