...
@section('header')
    <div>
        ...
        <div x-show="returnEquipmentData" class="mt-4 bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h3 class="text-lg font-medium text-blue-900 mb-2">返却対象機材</h3>
            <div class="flex items-center gap-3">
                <div class="text-sm font-medium text-blue-800" x-text="returnEquipmentData?.equipmentName"></div>
                <div x-show="returnEquipmentData?.companyNumber"
                     class="px-2 py-1 border border-blue-300 rounded text-xs text-blue-700 bg-blue-100"
                     x-text="returnEquipmentData?.companyNumber"></div>
                <div class="text-sm text-blue-600">
                    基本倉庫ID: <span x-text="returnEquipmentData?.locationId"></span>
                </div>
            </div>
        </div>
    </div>
...
@section('content')
    <div class="p-6" x-data="returnSelectManager()">
        <!-- フィルター（セッションストレージから来た場合は非表示） -->
        <div x-show="!returnEquipmentData" class="mb-6 bg-gray-50 p-4 rounded-lg">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <!-- 基本倉庫フィルタ -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">基本倉庫ID</label>
            ...
            <div class="px-6 py-4 border-b border-gray-200">
                <div class="flex justify-between items-center">
                    ...
                    <div class="text-sm text-gray-600">
                        <span class="font-medium text-blue-600" x-text="pendingReturnsCount"></span>件の返却予定
                    </div>
                </div>
            </div>
                        ...
                        <template x-for="equipment in equipmentData" :key="equipment.id">
                            <tr class="hover:bg-gray-50"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div class="text-sm text-gray-600" x-text="equipment.location?.name || '不明'"></div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <select :value="getReturnDestination(equipment.id)"
                                        ...
                                        <option :value="equipment.location_id"
                                                ...
                                                x-text="`${equipment.location?.name || 'ID:' + equipment.location_id} (基本倉庫)`">
                                        </option>
            ...
            <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white" @click.stop>
                <div class="mt-3">
                    ...
                    <div class="flex items-center justify-between pb-3 border-b">
                        <h3 class="text-lg leading-6 font-medium text-gray-900">一括返却の確認</h3>
                    ...
                    <!-- 返却予定一覧 -->
                    <div class="py-4">
                        <p class="text-sm text-gray-600 mb-4">以下の機材を指定した返却先へ移動します。よろしいですか？</p>
