...
<div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 p-4">
    <div class="max-w-6xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white mb-1 sm:mb-2">
                        <i class="fas fa-cloud-upload-alt text-blue-600 dark:text-blue-400 mr-2"></i>
                        バックアップ管理
                    </h1>
                    <p class="text-sm sm:text-base text-gray-600 dark:text-gray-400">データベースとファイルのDropboxバックアップを管理します</p>
                </div>
                <div class="text-left sm:text-right">
                    <div class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">管理者専用機能</div>
                    <div class="text-xs text-gray-400 dark:text-gray-500">{{ now()->format('Y-m-d H:i:s') }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6">
                <h2 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white mb-4">
                    <i class="fab fa-dropbox text-blue-600 dark:text-blue-400 mr-2"></i>
                    Dropbox認証状態
                </h2>

                <div id="auth-status" class="space-y-4">
                    @if($isAuthenticated)
                        <div class="flex items-center p-4 bg-green-50 dark:bg-green-900/30 rounded-lg border border-green-200 dark:border-green-700">
                            <div class="flex-shrink-0">
                                <i class="fas fa-check-circle text-green-500 dark:text-green-400 text-xl"></i>
                            </div>
                            <div class="ml-3 flex-1">
                                <p class="text-sm font-medium text-green-800 dark:text-green-300">Dropbox認証済み</p>
                        ...
                        @if($authStatus && $authStatus['success'] && isset($authStatus['token_info']))
                            ...
                            <div class="space-y-3 mt-4">
                                <div class="bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-700 p-4">
                                    ...
                                    <div class="space-y-2">
                                        <div class="flex items-center justify-between">
                                            <span class="text-sm text-blue-800 dark:text-blue-300">アクセストークン期限:</span>
                                            ...
                                            @else
                                                <span class="text-sm text-gray-500 dark:text-gray-400">無期限</span>
                                            @endif
                                        </div>
                                        ...
                                        @if($tokenInfo['last_refreshed_at'])
                                            <div class="flex items-center justify-between border-t border-blue-200 dark:border-blue-600 pt-2 mt-2">
                                                <span class="text-sm text-blue-700 dark:text-blue-400">最終更新:</span>
                                        ...
                                        <!-- リフレッシュトークン状態 -->
                                        <div class="flex items-center justify-between border-t border-blue-200 dark:border-blue-600 pt-2">
                                            <span class="text-sm text-blue-700 dark:text-blue-400">リフレッシュトークン:</span>
                                ...
                                @if($tokenInfo['is_access_token_expired'])
                                    <div class="flex items-center p-3 bg-red-50 dark:bg-red-900/30 rounded-lg border border-red-200 dark:border-red-700">
                                ...
                                @elseif($tokenInfo['will_expire_soon'])
                                    <div class="flex items-center p-3 bg-yellow-50 dark:bg-yellow-900/30 rounded-lg border border-yellow-200 dark:border-yellow-700">
                                        <i class="fas fa-clock text-yellow-500 dark:text-yellow-400 mr-2"></i>
                    ...
                    @else
                        <div class="flex items-center p-4 bg-yellow-50 dark:bg-yellow-900/30 rounded-lg border border-yellow-200 dark:border-yellow-700">
                            <div class="flex-shrink-0">
                                <i class="fas fa-exclamation-triangle text-yellow-500 dark:text-yellow-400 text-xl"></i>
                            </div>
                            <div class="ml-3">
                                <p class="text-sm font-medium text-yellow-800 dark:text-yellow-300">Dropbox認証が必要です</p>
                                <p class="text-sm text-yellow-700 dark:text-yellow-400">バックアップを実行するには認証を完了してください</p>
                            </div>
                        </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 sm:p-6">
                <h2 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white mb-4">
                    <i class="fas fa-play-circle text-green-600 dark:text-green-400 mr-2"></i>
                    バックアップ実行
                </h2>

                <div class="space-y-4">
                    <div class="p-4 bg-blue-50 dark:bg-blue-900/30 rounded-lg border border-blue-200 dark:border-blue-700">
                        <h3 class="font-medium text-blue-900 dark:text-blue-300 mb-2">バックアップに含まれるもの:</h3>
                    ...
                    <div class="flex items-center space-x-2">
                        <input type="checkbox" id="upload-to-dropbox" checked class="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500">
                        <label for="upload-to-dropbox" class="text-sm text-gray-700 dark:text-gray-300">Dropboxにアップロード</label>
                    </div>
                    ...
                    @if(!$isAuthenticated)
                        <p class="text-sm text-gray-500 dark:text-gray-400 text-center">※ Dropbox認証が必要です</p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <!-- バックアップ結果 -->
        <div id="backup-results" class="hidden bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 mt-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg mt-6">
            <div class="border-b border-gray-200 dark:border-gray-700">
                <nav class="-mb-px flex space-x-4 sm:space-x-8 px-4 sm:px-6">
                    <button id="backup-tab" class="tab-button active py-3 sm:py-4 px-1 border-b-2 border-blue-500 font-medium text-xs sm:text-sm text-blue-600 dark:text-blue-400 whitespace-nowrap">
                        <i class="fas fa-cloud-upload-alt mr-1 sm:mr-2"></i>バックアップ履歴
                    </button>
                    <button id="restore-tab" class="tab-button py-3 sm:py-4 px-1 border-b-2 border-transparent font-medium text-xs sm:text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600 whitespace-nowrap">
            ...
            <div id="backup-content" class="tab-content p-4 sm:p-6">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
                        <i class="fas fa-history text-indigo-600 dark:text-indigo-400 mr-1 sm:mr-2"></i>
                        バックアップ履歴
                    </h2>
            ...
            <div id="restore-content" class="tab-content p-4 sm:p-6 hidden">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                    <h2 class="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
                        <i class="fas fa-cloud-download-alt text-red-600 dark:text-red-400 mr-1 sm:mr-2"></i>
                        データ復元
                    </h2>
                ...
                <div class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-lg p-4 mb-6">
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-red-800 dark:text-red-300">重要な注意事項</h3>
    ...
    <!-- ローディングモーダル -->
    <div id="loading-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-sm mx-4">
            <div class="text-center">
                <i class="fas fa-spinner fa-spin text-blue-600 dark:text-blue-400 text-3xl mb-4"></i>
                <p id="loading-message" class="text-gray-900 dark:text-white font-medium mb-2">バックアップ実行中...</p>
                <p class="text-gray-600 dark:text-gray-400 text-sm">しばらくお待ちください</p>
            </div>
        </div>
    </div>

    <!-- 復元確認モーダル -->
    <div id="restore-confirm-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-lg mx-4">
            <div class="text-center mb-6">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">データ復元の確認</h3>
            </div>
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    ...
                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">復元前に現在のデータをバックアップする（強く推奨）</span>
                </label>
            </div>

            <div class="flex space-x-3">
                <button id="cancel-restore" class="flex-1 px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-400 dark:hover:bg-gray-500 transition-colors">
                    キャンセル
                </button>
    ...
    <div class="env-backup-card bg-red-50 dark:bg-red-900/30 border-2 border-red-200 dark:border-red-700 rounded-lg p-4 sm:p-6 mt-8">
        <h3 class="text-base sm:text-lg font-semibold text-red-800 dark:text-red-300 mb-4">
            <i class="fas fa-lock mr-2"></i>環境設定(.env)の暗号化バックアップ
        </h3>

        <!-- 警告メッセージ -->
        <div class="bg-yellow-100 dark:bg-yellow-900/30 border-l-4 border-yellow-500 dark:border-yellow-600 p-4 mb-6">
            <div class="flex">
                <div class="flex-shrink-0">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="backup-section">
                <h4 class="font-medium text-gray-900 dark:text-white mb-3">新規バックアップ作成</h4>
            ...
            <div class="restore-section">
                <h4 class="font-medium text-gray-900 dark:text-white mb-3">バックアップから復元</h4>
        ...
        <!-- 最新バックアップ情報 -->
        <div id="latest-env-backup" class="mt-4 text-sm text-gray-600 dark:text-gray-400">
            <i class="fas fa-info-circle mr-1"></i>読み込み中...
        </div>
    </div>

    <!-- 環境設定バックアップモーダル -->
    <div id="env-backup-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md mx-4 w-full">
            <div class="text-center mb-6">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">環境設定の暗号化バックアップ</h3>
            </div>

            <div class="space-y-4">
                <!-- パスワード入力 -->
                <div>
                    <label for="env-backup-password" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        暗号化パスワード
                    </label>
                    <input type="password" id="env-backup-password"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500"
                ...
                <!-- パスワード確認 -->
                <div>
                    <label for="env-backup-password-confirm" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        パスワード確認
                    </label>
                ...
                <!-- エラーメッセージ -->
                <div id="env-backup-error" class="hidden text-sm text-red-600 dark:text-red-400"></div>
            </div>

            <!-- ボタン -->
            <div class="flex space-x-3 mt-6">
                <button id="cancel-env-backup" class="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    キャンセル
                </button>
    ...
    <!-- 環境設定復元モーダル -->
    <div id="env-restore-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-70 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-2xl mx-4 w-full max-h-screen overflow-y-auto">
            <div class="text-center mb-6">
                ...
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">環境設定の復元</h3>
            </div>

            <div class="space-y-4">
                <!-- バックアップファイル選択 -->
                <div>
                    <label for="env-restore-file" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        復元するバックアップファイル
                    </label>
                    <select id="env-restore-file" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500">
                ...
                <div id="env-restore-password-section">
                    <label for="env-restore-password" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        復号化パスワード
                    </label>
                    <input type="password" id="env-restore-password"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500"
                           placeholder="バックアップ時のパスワード"
                           autocomplete="off">
                    <div id="restore-attempts-warning" class="hidden mt-2 text-sm text-red-600 dark:text-red-400">
                ...
                <div class="space-y-3">
                    ...
                    <label class="flex items-center">
                        <input type="checkbox" id="env-preview-only"
                               class="rounded border-gray-300 dark:border-gray-600 text-orange-600 shadow-sm focus:border-orange-300 focus:ring focus:ring-orange-200 focus:ring-opacity-50">
                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">プレビューのみ（適用しない）</span>
                    </label>
                </div>

                <!-- プレビュー表示エリア -->
                <div id="env-preview-section" class="hidden">
                    <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">環境設定プレビュー</h4>
                    <div class="border border-gray-200 dark:border-gray-600 rounded-md p-3 bg-gray-50 dark:bg-gray-700 max-h-60 overflow-y-auto">
                        <div id="env-preview-content" class="text-sm font-mono dark:text-gray-300"></div>
                    </div>
                </div>

                <!-- エラーメッセージ -->
                <div id="env-restore-error" class="hidden text-sm text-red-600 dark:text-red-400"></div>
            </div>

            <!-- ボタン -->
            <div class="flex space-x-3 mt-6">
                <button id="cancel-env-restore" class="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    キャンセル
                </button>
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        .then(data => {
            ...
            if (data.success && data.backups.length > 0) {
                listContainer.innerHTML = data.backups.map(backup => `
                    <div class="flex items-center justify-between p-3 sm:p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border dark:border-gray-600">
                        <div class="font-medium text-gray-900 dark:text-white text-sm sm:text-base">
                            <i class="fas fa-folder text-blue-500 dark:text-blue-400 mr-2"></i>${backup.name}
                        </div>
                    </div>
                `).join('');
            } else if (data.success) {
                listContainer.innerHTML = '<div class="text-center text-gray-500 dark:text-gray-400 py-8">バックアップが見つかりません</div>';
            } else {
                listContainer.innerHTML = '<div class="text-center text-red-500 dark:text-red-400 py-8">エラー: ' + data.error + '</div>';
            }
        })
        .catch(error => {
            document.getElementById('backup-list').innerHTML = '<div class="text-center text-red-500 dark:text-red-400 py-8">読み込みエラー: ' + error.message + '</div>';
        });
    }
    ...
    function showBackupResults(data) {
        ...
        let html = `<div class="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-lg p-4 mb-4">
            <div class="flex items-center">
                <i class="fas fa-check-circle text-green-500 dark:text-green-400 mr-2"></i>
                <span class="font-medium text-green-800 dark:text-green-300">バックアップ完了: ${data.timestamp}</span>
            </div>
        </div>`;
        ...
        if (data.results) {
            ...
            Object.entries(data.results).forEach(([type, result]) => {
                if (typeof result === 'object' && result.success !== undefined) {
                    const icon = result.success ? 'fas fa-check text-green-500 dark:text-green-400' : 'fas fa-times text-red-500 dark:text-red-400';
                    const bgColor = result.success ? 'bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-700' : 'bg-red-50 dark:bg-red-900/30 border-red-200 dark:border-red-700';

                    html += `<div class="p-3 ${bgColor} border rounded">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <i class="${icon} mr-2"></i>
                                <span class="font-medium dark:text-white">${type.toUpperCase()}</span>
                            </div>
                            ${result.size ? `<span class="text-sm text-gray-600 dark:text-gray-400">${(result.size / 1024 / 1024).toFixed(2)} MB</span>` : ''}
                        </div>
                        ${result.dropbox_path ? `<div class="text-sm text-gray-600 dark:text-gray-400 mt-1">Dropbox: ${result.dropbox_path}</div>` : ''}
                    </div>`;
                }
            });
            html += '</div>';
    ...
    function loadRestorableBackups() {
        ...
        .then(data => {
            ...
            if (data.success && data.backups.length > 0) {
                listContainer.innerHTML = data.backups.map(backup => `
                    <div class="p-3 sm:p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <div class="flex-shrink-0">
                                ...
                                ` : `
                                    <span class="px-3 sm:px-4 py-2 bg-gray-300 dark:bg-gray-600 text-gray-500 dark:text-gray-400 text-xs sm:text-sm rounded-lg cursor-not-allowed">
                                        復元不可
                                    </span>
                                `}
                            </div>
                ...
                listContainer.innerHTML = '<div class="text-center text-gray-500 dark:text-gray-400 py-8">復元可能なバックアップが見つかりません</div>';
            } else {
        ...
        .catch(error => {
            ...
            listContainer.innerHTML = '<div class="text-center text-red-500 dark:text-red-400 py-8">読み込みエラー: ' + error.message + '</div>';
        });
    }
