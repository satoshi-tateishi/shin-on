...
@section('breadcrumb')
    > <a href="{{ route('phases.equipment.show', [$phase, $phaseEquipment]) }}" class="text-blue-600 hover:text-blue-800">機材詳細</a>
...
@section('header')
    <div class="w-full">
        ...
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900 mb-2">{{ $phase->name }} 機材使用編集</h1>
...
<div class="p-3 sm:p-6 space-y-4 sm:space-y-6">
    <div class="bg-white border border-gray-200 rounded-lg">
        <div class="px-4 py-4 sm:p-6">
            <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-4 sm:mb-6">使用数量の編集</h3>
                ...
                <div class="mb-4 sm:mb-6 bg-red-50 border border-red-200 rounded-md p-3 sm:p-4">
                    <div class="flex">
                        ...
                        <div class="ml-2 sm:ml-3">
                            <h3 class="text-xs sm:text-sm font-medium text-red-800">入力エラーがあります</h3>
            ...
            <form method="POST" action="{{ route('phases.equipment.update', [$phase, $phaseEquipment]) }}">
                ...
                <!-- 現在の機材情報（編集不可） -->
                <div class="mb-4 sm:mb-6 bg-gray-50 border border-gray-200 rounded-lg p-3 sm:p-4">
                    <div class="grid grid-cols-2 gap-3 sm:gap-4">
                        <div>
                            <dt class="text-xs sm:text-sm font-medium text-gray-500">機材名</dt>
                            <dd class="mt-1 text-xs sm:text-sm text-gray-900">{{ $phaseEquipment->equipment->name }}</dd>
                        </div>
                        @if($phaseEquipment->equipment->company_number)
                            <div>
                                <dt class="text-xs sm:text-sm font-medium text-gray-500">新音番号</dt>
                                <dd class="mt-1 text-xs sm:text-sm text-gray-900">{{ $phaseEquipment->equipment->company_number }}</dd>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="space-y-4 sm:space-y-6">
                    <!-- 数量 -->
                    <div>
                        <label for="quantity" class="block text-xs sm:text-sm font-medium text-gray-700">使用数量</label>
                        <div class="mt-1">
                            @if($phaseEquipment->equipment->management_type === 'individual')
                                <input type="hidden" name="quantity" value="1">
                                <div class="mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm text-xs sm:text-sm text-gray-900">
                                    1個（個体管理機材）
                                </div>
                            @else
                                <input type="number" name="quantity" id="quantity"
                                       ...
                                       class="mt-1 block w-20 sm:w-24 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm">
                    ...
                    @if($hasConflicts)
                        <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 sm:p-4">
                            <div class="flex">
                                ...
                                <div class="ml-2 sm:ml-3">
                                    <h3 class="text-xs sm:text-sm font-medium text-yellow-800">期間重複の警告</h3>
                ...
                <div class="mt-6 sm:mt-8 flex justify-end gap-2 sm:space-x-3">
                    <a href="{{ route('phases.equipment.show', [$phase, $phaseEquipment]) }}"
                       class="px-3 sm:px-4 py-2 bg-gray-300 border border-transparent text-xs sm:text-sm font-medium rounded-md text-gray-700 hover:bg-gray-400">
                        戻る
                    </a>
                    <button type="submit"
    ...
    @if($otherUsages->count() > 0)
        <div class="bg-white border border-gray-200 rounded-lg">
            <div class="px-4 py-4 sm:p-6">
                <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-3 sm:mb-4">同一機材の他の使用予定</h3>
                        ...
                        <div class="border border-gray-200 rounded-lg p-3">
                            <div class="flex items-start justify-between gap-2">
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-medium text-gray-900 truncate">{{ $usage->phase->performance->title }}</p>
                                    <p class="text-xs text-gray-500">{{ $usage->phase->name }}</p>
                                </div>
                ...
                <div class="hidden sm:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-300">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">公演・フェーズ</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">期間</th>
                            ...
                            @foreach($otherUsages as $usage)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3">
                                        ...
                                        <div class="text-xs text-gray-500">{{ $usage->phase->name }}</div>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        <div>{{ $usage->phase->start_date->format('Y/m/d') }}</div>
                                        <div class="text-xs text-gray-500">～ {{ $usage->phase->end_date->format('Y/m/d') }}</div>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-900">
                                        {{ $usage->quantity }}個
                                    </td>
