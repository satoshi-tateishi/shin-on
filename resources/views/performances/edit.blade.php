...
<div class="p-3 sm:p-6">
    <form id="edit-form" method="POST" action="{{ route('performances.update', $performance) }}" enctype="multipart/form-data" class="space-y-4 sm:space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white border border-gray-200 rounded-lg">
            ...
            <div class="px-4 sm:px-6 py-3 sm:py-4">
                ...
                @error('attachments.*')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
                <div id="file-preview" class="mt-3 sm:mt-4 flex flex-wrap gap-2 hidden"></div>
            </div>
        </div>
        ...
        fileInput.addEventListener('change', function(e) {
            ...
            files.forEach((file, index) => {
                if (file.type.startsWith('image/')) {
                    ...
                    wrapper.className = 'w-20 h-20 sm:w-24 sm:h-24';

                    const imgElement = document.createElement('img');
                    imgElement.className = 'w-full h-full object-contain border border-gray-200 rounded';
