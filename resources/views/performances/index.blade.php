...
@section('breadcrumb')
    > <span class="text-gray-800 dark:text-gray-200">公演一覧</span>
@endsection

@section('header')
    <div class="w-full">
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">公演一覧</h1>
                ...
                <form method="GET" action="{{ route('performances.index') }}" class="space-y-3 sm:space-y-4">
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                        <div class="col-span-2 sm:col-span-1">
                            <label for="search" class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">公演名</label>
                        ...
                        <div>
                            <label for="performance_type" class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">種別</label>
                        ...
                        <div>
                            <label for="phase_status" class="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">フェーズ状態</label>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">公演情報</th>
                            <th class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden md:table-cell">期間</th>
                            <th class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider hidden lg:table-cell">会場</th>
                            <th class="px-3 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">フェーズ状態</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($performances as $performance)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 active:bg-gray-100 dark:active:bg-gray-600 cursor-pointer" onclick="window.location='{{ route('performances.show', $performance) }}'">
                                <td class="px-3 sm:px-6 py-3 sm:py-4">
                                    <div>
                                        <div class="text-xs sm:text-sm font-medium text-gray-900 dark:text-white">{{ $performance->title }}</div>
                                        @if($performance->director)
                                            <div class="text-xs text-gray-400 dark:text-gray-500">演出: {{ $performance->director }}</div>
                                        @endif
                                        <!-- モバイル用: 期間表示 -->
                                ...
                                <td class="px-3 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-900 dark:text-white hidden md:table-cell">
                                    @if($performance->start_date && $performance->end_date)
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400">〜 {{ $performance->end_date }}</div>
                                        @if($performance->duration_days)
                                            <div class="text-xs text-gray-400 dark:text-gray-500">({{ $performance->duration_days }}日間)</div>
                                        @endif
                                    @elseif($performance->phases_count > 0)
                                        <div class="text-gray-500 dark:text-gray-400 text-xs">フェーズで設定</div>
                                    @else
                                        <div class="text-gray-400 dark:text-gray-500 text-xs">期間未設定</div>
                                    @endif
                                </td>
                                <td class="px-3 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-900 dark:text-white hidden lg:table-cell">
                                    @if($performance->phases->count() > 0)
                                        <div class="space-y-1">
                                            @foreach($performance->phases->sortBy('start_date')->take(2) as $phase)
                                                <div class="text-xs">
                                                    <span class="font-medium text-gray-700 dark:text-gray-300">{{ $phase->name }}:</span>
                                                    <span>{{ $phase->location ? $phase->location->name : '未設定' }}</span>
                                                </div>
                                            @endforeach
                                            @if($performance->phases->count() > 2)
                                                <div class="text-xs text-gray-400 dark:text-gray-500">他{{ $performance->phases->count() - 2 }}件</div>
                                            @endif
                                        </div>
                                    @else
                                        <span class="text-gray-400 dark:text-gray-500 text-xs">フェーズ未設定</span>
                                    @endif
                                </td>
                                <td class="px-3 sm:px-6 py-3 sm:py-4">
                                            ...
                                            <div class="space-y-1">
                                                @foreach($performance->phases->sortBy('start_date') as $phase)
                                                    ...
                                                    <div class="flex items-center gap-1.5 whitespace-nowrap">
                                                        ...
                                                        <span class="text-xs text-gray-500 dark:text-gray-400">{{ $phase->name }}</span>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @else
                                            <span class="text-xs text-gray-400 dark:text-gray-500">フェーズなし</span>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 sm:px-6 py-8 sm:py-12 text-center">
                                    ...
                                    <h3 class="mt-2 text-xs sm:text-sm font-medium text-gray-900 dark:text-white">公演がありません</h3>
                                    <p class="mt-1 text-xs sm:text-sm text-gray-500 dark:text-gray-400">公演が登録されていません。</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
