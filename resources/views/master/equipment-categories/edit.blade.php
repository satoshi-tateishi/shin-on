...
@section('header')
    <div>
        <h1 class="text-3xl font-bold text-gray-900">機材カテゴリ編集</h1>
        <p class="mt-1 text-sm text-gray-600">機材カテゴリ「{{ $equipmentCategory->name }}」を編集します。</p>
    </div>
...
@section('content')
    <div class="p-6">
        <form method="POST" action="{{ route('master.equipment-categories.update', $equipmentCategory) }}" class="max-w-2xl">
            @csrf
            @method('PUT')
