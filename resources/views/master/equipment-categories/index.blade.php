...
@section('title', '機材カテゴリマスタ')

@section('breadcrumb')
    > <span class="text-gray-400 dark:text-gray-500">機材関連マスタ</span> > <span class="text-gray-800 dark:text-gray-200">機材カテゴリマスタ 一覧</span>
@endsection

@section('header')
    <div>
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white">機材カテゴリマスタ</h1>
    </div>
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <div class="py-1">
                        ...
                        <button type="button" @click="open = false; window.dispatchEvent(new CustomEvent('open-csv-import-modal', { detail: { actionUrl: '{{ route('master.equipment-categories.import-csv') }}' }}))"
    ...
    <div class="p-3 sm:p-6">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody id="sortable-tbody" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($categories as $category)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 @if(in_array(auth()->user()->role, ['editor', 'admin'])) sortable-row @endif @if(auth()->user()->role !== 'admin') cursor-pointer @endif" data-id="{{ $category->id }}" @if(auth()->user()->role !== 'admin') onclick="window.location.href='{{ route('master.equipment-categories.show', $category) }}'" @endif>
                                @if(in_array(auth()->user()->role, ['editor', 'admin']))
                                ...
                                @endif
                                <td class="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900 dark:text-white text-center @if(auth()->user()->role === 'admin') cursor-pointer @endif" @if(auth()->user()->role === 'admin') onclick="window.location.href='{{ route('master.equipment-categories.show', $category) }}'" @endif>
                                    {{ $category->sort }}
                                </td>
                                <td class="px-2 sm:px-4 py-2 @if(auth()->user()->role === 'admin') cursor-pointer @endif" @if(auth()->user()->role === 'admin') onclick="window.location.href='{{ route('master.equipment-categories.show', $category) }}'" @endif>
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">
                                        {{ $category->name }}
                                    </div>
                                </td>
                                <td class="px-2 sm:px-4 py-2 text-center @if(auth()->user()->role === 'admin') cursor-pointer @endif" @if(auth()->user()->role === 'admin') onclick="window.location.href='{{ route('master.equipment-categories.show', $category) }}'" @endif>
                                    <span class="inline-flex px-1.5 py-0.5 text-xs font-semibold rounded-full
                                        {{ $category->is_active ? 'bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-300' }}">
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">新しいカテゴリを作成してください。</p>
                @if(auth()->user()->role === 'editor' || auth()->user()->role === 'admin')
                    <div class="mt-6">
                        <a href="{{ route('master.equipment-categories.create') }}"
