...
@section('breadcrumb')
    > <span class="text-gray-400">機材関連マスタ</span> > <span class="text-gray-800">機材カテゴリマスタ 一覧</span>
@endsection

@section('header')
    <div>
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900">機材カテゴリマスタ</h1>
    </div>
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <div class="py-1">
                        ...
                        <button type="button" @click="open = false; showImportModal('{{ route('master.equipment-categories.import-csv') }}')"
    ...
    <div class="p-3 sm:p-6">
                    ...
                    <tbody id="sortable-tbody" class="bg-white divide-y divide-gray-200">
                        @foreach($categories as $category)
                            <tr class="hover:bg-gray-50 @if(in_array(auth()->user()->role, ['editor', 'admin'])) sortable-row @endif @if(auth()->user()->role !== 'admin') cursor-pointer @endif" data-id="{{ $category->id }}" @if(auth()->user()->role !== 'admin') onclick="window.location.href='{{ route('master.equipment-categories.show', $category) }}'" @endif>
                                ...
                                @endif
                                <td class="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900 text-center @if(auth()->user()->role === 'admin') cursor-pointer @endif" @if(auth()->user()->role === 'admin') onclick="window.location.href='{{ route('master.equipment-categories.show', $category) }}'" @endif>
                                    {{ $category->sort }}
                                </td>
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500">新しいカテゴリを作成してください。</p>
                @if(auth()->user()->role === 'editor' || auth()->user()->role === 'admin')
                    <div class="mt-6">
                        <a href="{{ route('master.equipment-categories.create') }}"
