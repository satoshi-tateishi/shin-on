...
@section('header')
    <div>
        <h1 class="text-3xl font-bold text-gray-900">使用場所マスタ編集</h1>
        <p class="mt-1 text-sm text-gray-600">使用場所「{{ $location->name }}」の情報を編集します。</p>
    </div>
...
@section('content')
    <div class="p-6">
        <form method="POST" action="{{ route('master.locations.update', $location) }}" class="max-w-4xl">
            @csrf
            @method('PUT')
