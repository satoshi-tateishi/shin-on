...
@section('breadcrumb')
    > <a href="{{ route('equipment-subcategories.index') }}" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">機材サブカテゴリ 一覧</a>
...
@section('header')
    <div class="w-full">
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">{{ $equipmentSubcategory->name }} - 編集</h1>
        <div class="flex items-center justify-between">
            <a href="{{ route('master.equipment-subcategories.show', $equipmentSubcategory) }}"
...
@section('content')
    <div class="p-3 sm:p-6">
        <form id="edit-form" method="POST" action="{{ route('master.equipment-subcategories.update', $equipmentSubcategory) }}" class="max-w-2xl">
            @csrf
            @method('PUT')
