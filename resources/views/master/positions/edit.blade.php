...
@section('header')
    <div>
        <h1 class="text-3xl font-bold text-gray-900">ポジションマスタ編集</h1>
        <p class="mt-1 text-sm text-gray-600">ポジション「{{ $position->name }}」を編集します。</p>
    </div>
...
@section('content')
    <div class="p-6">
        <form method="POST" action="{{ route('master.positions.update', $position) }}" class="max-w-2xl">
            @csrf
            @method('PUT')
