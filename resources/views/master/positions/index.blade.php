...
@section('breadcrumb')
    > <span class="text-gray-400 dark:text-gray-500">マスタ管理</span> > <span class="text-gray-800 dark:text-gray-200">ポジション</span>
@endsection

@section('header')
    <div>
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900 dark:text-white">ポジションマスタ</h1>
    </div>
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.outside="open = false" x-transition
                    ...
                    <div class="py-1">
                        ...
                        <button type="button" @click="open = false; window.dispatchEvent(new CustomEvent('open-csv-import-modal', { detail: { actionUrl: '{{ route('master.positions.import-csv') }}' }}))"
    ...
    <div class="p-3 sm:p-6">
        ...
        <div class="mb-4 sm:mb-6">
            <div class="flex space-x-2 sm:space-x-4">
                <a href="{{ route('master.positions.index', ['is_active' => '1']) }}"
                   class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors {{ request('is_active', '1') == '1' ? 'bg-green-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                    有効
                </a>
                <a href="{{ route('master.positions.index', ['is_active' => '0']) }}"
                   class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors {{ request('is_active') == '0' ? 'bg-red-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700' }}">
                    無効
                </a>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody id="sortable-tbody" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($positions as $position)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 @if(in_array(auth()->user()->role, ['editor', 'admin'])) sortable-row @endif @if(auth()->user()->role !== 'admin') cursor-pointer @endif" data-id="{{ $position->id }}" @if(auth()->user()->role !== 'admin') onclick="window.location.href='{{ route('master.positions.show', $position) }}'" @endif>
                                @if(in_array(auth()->user()->role, ['editor', 'admin']))
                                ...
                                @endif
                                <td class="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900 dark:text-white text-center @if(auth()->user()->role === 'admin') cursor-pointer @endif" @if(auth()->user()->role === 'admin') onclick="window.location.href='{{ route('master.positions.show', $position) }}'" @endif>
                                    {{ $position->sort }}
                                </td>
                                <td class="px-3 sm:px-4 py-2 @if(auth()->user()->role === 'admin') cursor-pointer @endif" @if(auth()->user()->role === 'admin') onclick="window.location.href='{{ route('master.positions.show', $position) }}'" @endif>
        ...
        @else
            <div class="text-center py-12">
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">検索条件を変更するか、新しいポジションを作成してください。</p>
                @if(auth()->user()->role === 'editor' || auth()->user()->role === 'admin')
                    <div class="mt-6">
                        <a href="{{ route('master.positions.create') }}"
