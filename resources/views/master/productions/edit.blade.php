...
@section('header')
    <div>
        <h1 class="text-3xl font-bold text-gray-900">プロダクションマスタ編集</h1>
        <p class="mt-1 text-sm text-gray-600">プロダクション「{{ $production->name }}」の情報を編集します。</p>
    </div>
...
@section('content')
    <div class="p-6">
        <form method="POST" action="{{ route('master.productions.update', $production) }}" class="max-w-2xl">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="address" id="address" rows="3"
                              class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500
                                     @error('address') border-red-300 @enderror">{{ old('address', $production->address) }}</textarea>
                    @error('address')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
