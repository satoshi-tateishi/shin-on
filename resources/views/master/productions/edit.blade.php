...
@section('header')
    <div class="w-full">
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900 mb-2">{{ $production->name }} - 編集</h1>
        <div class="flex items-center justify-between">
            <a href="{{ route('master.productions.show', $production) }}"
...
@section('content')
    <div class="p-3 sm:p-6">
        <form id="edit-form" method="POST" action="{{ route('master.productions.update', $production) }}" class="max-w-2xl">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4 sm:space-y-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        @foreach($types as $type)
                            <option value="{{ $type }}" {{ old('type', $production->type) == $type ? 'selected' : '' }}>{{ $type }}</option>
                        @endforeach
                    </select>
                    @error('type')
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $production->name) }}" required
                           class="w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500
                ...
                <div>
                    ...
                    <input type="text" name="postal_code" id="postal_code" value="{{ old('postal_code', $production->postal_code) }}"
                ...
                <div>
                    ...
                    <input type="text" name="address" id="address" value="{{ old('address', $production->address) }}"
                           class="w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500
                ...
                <!-- 備考 -->
                <div>
                    <label for="note" class="block text-xs sm:text-sm font-medium text-gray-700 mb-1">
                        備考
                    </label>
                    <textarea name="note" id="note" rows="2"
                              class="w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500
                                     @error('note') border-red-300 @enderror">{{ old('note', $production->note) }}</textarea>
                    @error('note')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                               {{ old('is_active', $production->is_active) ? 'checked' : '' }}
