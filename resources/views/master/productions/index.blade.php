...
@section('header')
    <div>
        <h1 class="text-xl sm:text-3xl font-bold text-gray-900">プロダクションマスタ</h1>
    </div>
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.away="open = false" x-transition
                    ...
                    <div class="py-1">
                        ...
                        <button type="button" @click="open = false; showImportModal('{{ route('master.productions.import-csv') }}')"
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody id="sortable-tbody" class="bg-white divide-y divide-gray-200">
                        @foreach($productions as $production)
                            <tr class="hover:bg-gray-50 @if(in_array(auth()->user()->role, ['editor', 'admin']) && !request('search') && !request('is_active')) sortable-row @endif @if(auth()->user()->role !== 'admin') cursor-pointer @endif" data-id="{{ $production->id }}" @if(auth()->user()->role !== 'admin') onclick="window.location.href='{{ route('master.productions.show', $production) }}'" @endif>
                                @if(in_array(auth()->user()->role, ['editor', 'admin']) && !request('search') && !request('is_active'))
                                ...
                                @endif
                                @if(!request('search') && !request('is_active'))
                                    <td class="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900 text-center @if(auth()->user()->role === 'admin') cursor-pointer @endif" @if(auth()->user()->role === 'admin') onclick="window.location.href='{{ route('master.productions.show', $production) }}'" @endif>
                                        {{ $production->sort }}
                                    </td>
                                @endif
