    ...
    @if(auth()->user()->role === 'editor' || auth()->user()->role === 'admin')
        <div class="flex gap-2 sm:gap-3 ml-auto">
            @if(request('sort_mode') === 'all')
                <a href="{{ route('master.productions.index', array_merge(request()->query(), ['sort_mode' => null])) }}"
            ...
            @else
                <a href="{{ route('master.productions.index', array_merge(request()->query(), ['sort_mode' => 'all'])) }}"
...
@section('content')
    <div class="p-3 sm:p-6">
        ...
        @if($productions->count() > 0)
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody id="sortable-tbody" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        @foreach($productions as $production)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 @if(in_array(auth()->user()->role, ['editor', 'admin']) && request('sort_mode') === 'all') sortable-row @endif @if(auth()->user()->role !== 'admin') cursor-pointer @endif" data-id="{{ $production->id }}" @if(auth()->user()->role !== 'admin') onclick="window.location.href='{{ route('master.productions.show', $production) }}'" @endif>
                                @if(in_array(auth()->user()->role, ['editor', 'admin']) && request('sort_mode') === 'all')
                                ...
                                @if(request('sort_mode') !== 'all')
                                    <td class="px-2 py-2 whitespace-nowrap text-xs sm:text-sm text-gray-900 dark:text-white text-center @if(auth()->user()->role === 'admin') cursor-pointer @endif" @if(auth()->user()->role === 'admin') onclick="window.location.href='{{ route('master.productions.show', $production) }}'" @endif>
                                        {{ $loop->iteration + (method_exists($productions, 'currentPage') ? ($productions->currentPage() - 1) * $productions->perPage() : 0) }}
                                    </td>
                                @endif
            ...
            @if(request('sort_mode') !== 'all' && method_exists($productions, 'hasPages') && $productions->hasPages())
                <div class="mt-4 flex items-center justify-between">
                    <span class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                        {{ $productions->currentPage() }} / {{ ceil($totalProductions / 50) }} ページ
                    </span>
                    <div>
